...
export function genePrompt(g: Gene = loadBrain().gene): string {
  ...
  const morph =
    g.morph >= 2
      ? "Same FULL WHITE German Shepherd, no cape, same doors, same stones. Zero morph. Never tan, beige, grey, or saddle."
      : "Same full white German Shepherd, no cape, same doors. No morph. Never tan.";
...
export function stillLaws(): string {
  const b = loadBrain();
  const bits = ["No walking. No strides. The dog stays on the same floor tiles. Feet glued. FULL WHITE coat — no tan, beige, saddle, or mask."];
  if (b.camera >= 2) bits.push("Pillars, floor vanishing point and door arches match the start image pixel-for-pixel.");
  if (b.messy >= 2) bits.push("Do not morph the dog or the doors. Same FULL WHITE German Shepherd, no cape, no tan, same stone.");
  return bits.join(" ");
}
...
export function retryLaw(grade: Grade): string {
  ...
  if (grade === "messy") return "No morph. Same full-white dog, same doors, same hall. Only position changes. Never tan.";
  return "";
}
