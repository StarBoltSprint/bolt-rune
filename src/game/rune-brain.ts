...
export function genePrompt(g: Gene = loadBrain().gene): string {
  ...
  const morph =
    g.morph >= 2
      ? "Same WHITE German Shepherd, no cape, same doors, same stones. Zero morph. Never grey."
      : "Same white German Shepherd, no cape, same doors. No morph.";
  const dest = g.dest > 1 ? "Last frame: he is already standing at the destination, not still walking." : "";
  return [cam, `The wolf takes at least ${g.strides} full strides across the floor.`, morph, dest].filter(Boolean).join(" ");
}
...
export function stillLaws(): string {
  const b = loadBrain();
  const bits = ["No walking. No strides. The wolf stays on the same floor tiles."];
  if (b.camera >= 2) bits.push("Pillars, floor vanishing point and door arches match the start image pixel-for-pixel.");
  if (b.messy >= 2) bits.push("Do not morph the dog or the doors. Same WHITE German Shepherd, no cape, same stone.");
  return bits.join(" ");
}
...
export function brainLaws(): string {
  ...
  if (b.stuck >= 2) bits.push("Past wolves froze in place. This wolf MUST take many full strides and finish at the destination, not the start.");
  if (b.messy >= 2) bits.push("Do not morph the dog or the doors. Same WHITE German Shepherd, no cape, same stone.");
...
export function retryLaw(grade: Grade): string {
  if (grade === "camera") return "REJECT any camera move. Architecture is welded. Only the wolf translates.";
  if (grade === "stuck") return "The wolf MUST walk. At least eight strides. Last frame is a different place than frame one.";
  if (grade === "messy") return "No morph. Same wolf, same doors, same hall. Only position changes.";
  return "";
}
