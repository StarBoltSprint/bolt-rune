...
async function restart() {
  ...
  // Nitro's vercel bundle omits PGLite's wasm/data next to the chunk; local
  // preview has no DATABASE_URL so getSql() falls back to PGLite and dies
  // without these files. Deployed Vercel injects Neon and never hits this path.
  try {
    ...
    if (existsSync(dest) && existsSync(join(src, "pglite.data"))) {
    ...
    /* preview still starts; missing assets surface as a PGLite log */
  }
